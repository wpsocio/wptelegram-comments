...
export const jsPotToPhp = (cb) => {
	...
	} ${config.domainPath}/${config.textDomain}-js-translations.php ${pkg.name}`;
...
export const generatePotFile = (done) => {
	...
	wpi18n
		...
		.then(function (wpPackage) {
			console.log(
				"POT file saved to " +
					path.relative(wpPackage.getPath(), wpPackage.getPotFilename())
			);
		})
...
export const updateVersion = (done) => {
	...
	return (
		gulp
			...
			// add readme files
			.pipe(
				gulp.src(["./README.md", config.srcDir + "/README.txt"], srcOptions)
			)
